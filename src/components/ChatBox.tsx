...
export function ChatBox({
  ...
  return (
    <section className="rounded-lg border border-emerald-400/20 bg-black/35 p-4 shadow-lg shadow-emerald-950/20 sm:p-5">
      ...
      {errorMessage ? (
        <div className="mb-3 animate-float-in rounded-lg border border-rose-400/25 bg-rose-500/10 px-4 py-3 text-sm leading-6 text-rose-100">
          <p className="font-medium text-rose-200">出现了一点问题</p>
          <p className="mt-1 text-rose-100/90">{errorMessage}</p>
        </div>
      ) : null}

      <div className="flex max-h-[16rem] min-h-[11rem] flex-col gap-3 overflow-y-auto rounded-lg border border-stone-800/80 bg-stone-950/70 p-3 sm:max-h-[18rem] sm:min-h-[12rem]">
        {isEmpty ? (
          <div className="animate-float-in flex min-h-[9rem] flex-1 flex-col items-center justify-center rounded-lg border border-dashed border-emerald-400/15 bg-emerald-400/5 px-6 text-center">
            <div className="mb-3 flex h-10 w-10 items-center justify-center rounded-full border border-emerald-300/25 bg-emerald-400/10 text-emerald-200">
              ?
            </div>
      ...
      <div className="mt-3 rounded-lg border border-amber-400/15 bg-amber-400/5 px-4 py-2.5">
